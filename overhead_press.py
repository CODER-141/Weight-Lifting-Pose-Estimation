...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if not input_done:
            ...
            cv2.imshow('Overhead Press Workout', image)
            key = cv2.waitKey(1)
        ...
        try:
            ...
            # Use left arm: shoulder, elbow, wrist
            ...
            # Check correctness of form
            # Good overhead press: elbow angle between 70-180 degrees
            if 70 < angle < 180:
                feedback = "Good overhead press form!"
                color = (0, 255, 0)
            else:
            ...
            # Rep counting logic
            if not workout_done:
                # Start position: elbow bent (angle < 120 = bar at shoulder level)
                if angle < 120:
                    stage = "down"
                # Press up: arm extended (angle > 160)
        ...
        # Draw the UI box
        cv2.rectangle(image, (0, 0), (300, 80), (16, 117, 245), -1)
        ...
        if workout_done:
            cv2.putText(image, "Overhead press complete!", (80, 400),
